...
class vehicle2d:
    ...
    def draw_veh_body(self,z_up=-1,draw_vel=True):
        ...
        # draw center of gravity sign
        cg_sign(radius=0.1,ax=axes) 
        # draw yaw rate arc arrow
        if self.yaw_rate > 0:
            arc_arrow((0,0), 0.2, 0, 180, ax=axes,linewidth=0.5,z_up=z_up)
        else:
            arc_arrow((0,0), 0.2, 0,-180, ax=axes,linewidth=0.5,z_up=z_up)
        axes.text(0,0.4,r"$r$",color='k',fontsize=10)
        if draw_vel:
            self.draw_veh_vel(z_up=z_up)
        
    def draw_veh_vel(self,z_up=-1,vl_ratio=10):
        ...
        draw_vector_with_text(V_line_rotated,"k",text=r"$V$",l_text=0.3,z_up=z_up,turn_ang=5/6*math.pi)